...
export const updatePartnerProfilePic = async (
  req: MyCustomRequest,
  res: Response
) => {
  try {
...
export const kycUpdate = async (req: MyCustomRequest, res: Response) => {
  try {
    ...
    // const photo = req?.files?.photo?.[0]?.filename;
    // const { ...data } = req.body;
    const poi = (req.files as Record<string, Express.Multer.File[]>).poi?.[0]
      ?.filename;
    const poq = (req.files as Record<string, Express.Multer.File[]>).poq?.[0]
      ?.filename;
    const photo = (req.files as Record<string, Express.Multer.File[]>)
      .photo?.[0]?.filename;
    const { ...data } = req.body;
    // console.log(req.body.firstName);
    // console.log(req.id);
...
//   }
// };
export const kycDocumentUpload = async (
  req: MyCustomRequest,
  res: Response
) => {
  try {
